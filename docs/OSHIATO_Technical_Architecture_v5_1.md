...
| カテゴリ | 技術 | 用途 |
|----------|------|------|
| **マップ統合型検索** | Mapbox GL JS（既存）+ CSS | マップ上部に常時表示の検索バー。結果は下からスライドするパネルで表示（D-005） |
| **キーワード検索** | Supabase `.ilike()` | コメント・推し名・グループ名を ILIKE でクライアントサイド検索（D-002） |
| **現在地表示** | Mapbox `GeolocateControl` | 青いドット表示 + 現在地ボタン。カスタム実装を廃止して標準コントロールに統一（D-007） |
| **レコメンドアルゴリズム** | PostgreSQL（Supabase） | 協調フィルタリング：`user_oshis`テーブルを使ったSQL集計でスコアリング |
| **マップ追加描画** | Mapbox GL JS（既存） | おすすめスポットを別レイヤーのピンとして重ねて表示（Step 8 で実装予定） |

**設計方針:**
- 検索はタブ追加ではなくマップに統合（375px でタブ6本は窮屈。地図アプリとして自然な UX）
- 全文検索インデックス（`tsvector`）はデータ量が少ない現状では不要。Phase G で検討
- 近くのスポット専用セクションは廃止。GeolocateControl で代用（D-006）
