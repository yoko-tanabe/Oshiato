...
地図画面の期間フィルター下に配置。登録済みの推しを複数選択可能。

```
┌─────────────────┐
│ すべての推し  ▼  │  ← トリガーボタン（未選択時）
└─────────────────┘
│ 2人選択中    ▲  │  ← トリガーボタン（選択時・Primary色ボーダー）
├─────────────────┤
│ ● すべて        │  ← 全選択解除
│ 🟣 推しA     ✓  │  ← 選択中（推しカラー背景・チェックマーク）
│ 🩷 推しB     ✓  │
│ 🟢 推しC        │  ← 未選択
└─────────────────┘
```

- トリガー: `font-size: 12px`, `border-radius: 999px`, `backdrop-filter: blur(8px)`
- ドロップダウン: `background: rgba(30, 30, 30, 0.95)`, `border-radius: 12px`, `max-height: 240px`（スクロール可）
- 選択中の行: 推しカラー10%背景 + 推しカラー文字色 + ✓マーク
...
├─────────────────────────────┤
│ [すべて][今日][明日][今週]   │  ← 期間フィルター
│ [すべての推し ▼]            │  ← 推しフィルター（ドロップダウン）
│                             │
